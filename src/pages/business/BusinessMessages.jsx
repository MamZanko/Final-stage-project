...
import {
  ...
  EllipsisVerticalIcon, ArrowLeftIcon, XMarkIcon, DocumentIcon,
...
const BusinessMessages = () => {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      setLoading((prev) => prev); // don't flash loading on polls
  ...
  const fetchMessages = useCallback(async (convoId, showLoader = true) => {
    try {
      ...
      if (showLoader) console.error('Failed to fetch messages:', err);
    } finally {
  ...
  // Poll conversations every 10s to update unread badges & sidebar
  useEffect(() => {
  ...
  }, [selectedConvo?.id, fetchMessages, fetchConversations]);
  ...
  const filteredConvos = conversations.filter((c) =>
    !search.trim() || c.other_participant?.name?.toLowerCase().includes(search.toLowerCase())
  );
  ...
  const handleSend = async (e) => {
    e?.preventDefault?.();
    if ((!message.trim() && !selectedFile) || !selectedConvo) return;
    const text = message;
    ...
    const tempMsg = { id: Date.now(), body: text, sender_id: user?.id, sender: { id: user?.id }, created_at: new Date().toISOString(), is_read: false, file_name: file?.name };
    shouldScrollRef.current = true;
    ...
    try {
      ...
      setMessages((prev) => prev.map((m) => (m.id === tempMsg.id ? (res.data || res) : m)));
      fetchConversations();
    } catch (err) {
  ...
  const selectConvo = (convo) => { setSelectedConvo(convo); setShowMobileChat(true); };
  ...
  return (
    <motion.div {...pageTransition} className="flex-1 flex flex-col overflow-hidden">
      <div className="flex-1 flex overflow-hidden">
        ...
        {/* ═══════════ SIDEBAR ═══════════ */}
        <div className={`w-full md:w-[380px] lg:w-[420px] flex-shrink-0 border-r border-[var(--color-border)] bg-[var(--color-card-bg)] flex flex-col ${showMobileChat ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-20 px-6">
                ...
                <p className="text-sm text-[var(--color-text-secondary)] text-center max-w-[200px]">
                  {search ? 'Try a different keyword' : 'Messages from clients will appear here'}
                </p>
              </div>
            ) : (
              <div className="p-2">
          ...
          {selectedConvo ? (
            <>
                ...
                {loadingMessages ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-full">
                    ...
                    <p className="text-sm text-[var(--color-text-secondary)] max-w-xs text-center">
                      Send a message to {selectedConvo.other_participant?.name || 'this user'} to get started.
                    </p>
                  </div>
                ) : (
                                ...
                                >
                                  ...
                                  {msg.file_url && msg.file_type !== 'image' && (
                                    <a
                                      ...
                                      className={`flex items-center gap-3 p-3 rounded-xl mb-2 transition-colors ${isOwn ? 'bg-white/10 hover:bg-white/20' : 'bg-[var(--color-surface)] hover:bg-[var(--color-border)]'}`}
                                    >
                                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${isOwn ? 'bg-white/15' : 'bg-primary/10'}`}>
                                        <DocumentIcon className={`w-5 h-5 ${isOwn ? 'text-white' : 'text-primary'}`} />
                                      </div>
                                      <div className="min-w-0 flex-1">
                                        <p className="text-sm font-medium truncate">{msg.file_name || 'File'}</p>
                                        {msg.file_size && <p className={`text-xs mt-0.5 ${isOwn ? 'text-white/60' : 'text-[var(--color-text-secondary)]'}`}>{formatFileSize(msg.file_size)}</p>}
                                      </div>
                                    </a>
                                  )}
                                  {/* Text */}
                                  {(msg.body || msg.text) && <span>{msg.body || msg.text}</span>}
                                </div>
                                {/* Time & status */}
                                <div className={`flex items-center gap-1.5 mt-1.5 px-1 ${isOwn ? 'justify-end' : ''}`}>
                                  <span className="text-[11px] text-[var(--color-text-secondary)]">
                                    {new Date(msg.created_at || msg.timestamp).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                                  </span>
                                  {isOwn && (
              ...
              <div className="border-t border-[var(--color-border)] bg-[var(--color-card-bg)] flex-shrink-0">
                <form onSubmit={handleSend} className="px-4 sm:px-5 lg:px-6 py-4">
