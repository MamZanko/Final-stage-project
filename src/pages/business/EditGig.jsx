...
import { mockBusinessGigs } from '../../data/mockDataPhase3';
...
const EditGig = () => {
  ...
  const existingGig = mockBusinessGigs.find((g) => g.id === parseInt(id));
    ...
    const timer = setTimeout(() => {
      if (existingGig) {
        setForm({
          ...
          subcategory: '',
          description: existingGig.description || '<p>Professional service with high quality deliverables. Our team ensures the best results with timely delivery.</p>',
          images: existingGig.images || [existingGig.image],
          pricing: existingGig.pricing || form.pricing,
          tags: existingGig.tags || ['design', 'professional'],
          faq: existingGig.faq || [{ question: 'What is the delivery time?', answer: 'Depends on the package you choose.' }],
        });
      }
      setLoading(false);
    }, 600);
  ...
  const addTag = () => {
    ...
    if (t && !form.tags.includes(t) && form.tags.length < 10) {
      updateForm('tags', [...form.tags, t]); setNewTag('');
    }
  };
  const addFaq = () => updateForm('faq', [...form.faq, { question: '', answer: '' }]);
  const updateFaq = (idx, key, val) => {
    const newFaq = [...form.faq]; newFaq[idx][key] = val; updateForm('faq', newFaq);
  };

  const handleSave = () => {
    setSaving(true);
    setTimeout(() => { setSaving(false); navigate('/my-gigs'); }, 1500);
  };

  if (loading) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        <div className="h-8 bg-[var(--color-surface)] rounded-lg animate-shimmer w-48" style={{ backgroundSize: '200% 100%' }} />
        <div className="h-12 bg-[var(--color-surface)] rounded-xl animate-shimmer" style={{ backgroundSize: '200% 100%' }} />
        <div className="h-64 bg-[var(--color-surface)] rounded-2xl animate-shimmer" style={{ backgroundSize: '200% 100%' }} />
      </div>
    );
  }
  ...
  if (!existingGig) {
    return (
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-16 text-center">
        <p className="text-sm text-[var(--color-text-secondary)]">Gig not found.</p>
      </div>
    );
  }
  ...
  const gigPreview = {
    ...form,
    id: existingGig.id,
    image: form.images[0] || existingGig.image,
    startingPrice: form.pricing.basic.price || existingGig.startingPrice,
    rating: existingGig.rating,
    reviewCount: existingGig.reviewCount,
    business: existingGig.business,
  };
  ...
  return (
    <motion.div {...pageTransition} className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-heading font-bold text-[var(--color-text)]">Edit Gig</h1>
          <p className="text-sm text-[var(--color-text-secondary)] mt-0.5">{existingGig.title}</p>
        </div>
        ...
        {steps.map((step, i) => {
          ...
          return (
            <button key={step.key} onClick={() => setCurrentStep(i)} className="flex items-center gap-2 flex-shrink-0">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${isActive ? 'bg-primary text-white' : isDone ? 'bg-success text-white' : 'bg-[var(--color-surface)] text-[var(--color-text-secondary)]'}`}>
      ...
      {/* Step content — same as CreateGig */}
      <AnimatePresence mode="wait">
        {currentStep === 0 && (
          <motion.div key="overview" {...tabContent} className="space-y-5">
            <div>
              <label className="block text-sm font-medium text-[var(--color-text)] mb-1.5">Gig Title</label>
              <input type="text" value={form.title} onChange={(e) => updateForm('title', e.target.value)} maxLength={120} className="w-full px-4 py-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-card-bg)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-primary/40" />
              <p className="text-[10px] text-[var(--color-text-secondary)] mt-1">{form.title.length}/120</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="text" value={form.subcategory} onChange={(e) => updateForm('subcategory', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-[var(--color-border)] bg-[var(--color-card-bg)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-primary/40" />
              </div>
            </div>
          </motion.div>
        )}
        {currentStep === 1 && (
          <motion.div key="media" {...tabContent} className="space-y-5">
            ...
            }} label="Drop images or click to upload" sublabel="JPG, PNG, WEBP — max 5MB each" />
            {form.images.length > 0 && <ReorderableThumbnails images={form.images} onChange={(imgs) => updateForm('images', imgs)} maxImages={5} />}
          </motion.div>
        )}
        {currentStep === 2 && (
              ...
              <div key={tier} className="bg-[var(--color-card-bg)] border border-[var(--color-border)] rounded-2xl p-5">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <input type="number" value={form.pricing[tier].price} onChange={(e) => updatePricing(tier, 'price', e.target.value)} placeholder="Price $" className="px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-primary/40" />
                    <input type="number" value={form.pricing[tier].deliveryDays} onChange={(e) => updatePricing(tier, 'deliveryDays', e.target.value)} placeholder="Days" className="px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-primary/40" />
                    <input type="number" value={form.pricing[tier].revisions === -1 ? '' : form.pricing[tier].revisions} onChange={(e) => updatePricing(tier, 'revisions', e.target.value === '' ? -1 : parseInt(e.target.value))} placeholder="Revisions" className="px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-primary/40" />
                  </div>
                  <div>
                    {form.pricing[tier].features.map((feat, fi) => (
                      <div key={fi} className="flex gap-2 mb-1.5">
                        ...
                        {form.pricing[tier].features.length > 1 && <button onClick={() => removeFeature(tier, fi)} className="text-error text-xs">×</button>}
                      </div>
                    ))}
                    <button onClick={() => addFeature(tier)} className="text-xs text-primary">+ Add Feature</button>
                  </div>
                </div>
              </div>
            ))}
        ...
        {currentStep === 3 && (
          <motion.div key="description" {...tabContent}>
            <RichTextEditor value={form.description} onChange={(val) => updateForm('description', val)} maxLength={5000} minHeight={280} />
          </motion.div>
        )}
        {currentStep === 4 && (
          <motion.div key="extras" {...tabContent} className="space-y-6">
            <div>
              ...
              <div className="flex flex-wrap gap-2 mb-2">
                {form.tags.map((tag, i) => (
                  <span key={i} className="inline-flex items-center gap-1 px-2.5 py-1 bg-primary/5 text-primary text-xs rounded-full border border-primary/20">{tag}<button onClick={() => updateForm('tags', form.tags.filter((_, idx) => idx !== i))} className="hover:text-error">×</button></span>
                ))}
              </div>
              <div className="flex gap-2">
                <input type="text" value={newTag} onChange={(e) => setNewTag(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), addTag())} placeholder="Add tag..." className="flex-1 px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] text-sm text-[var(--color-text)] focus:outline-none focus:ring-2 focus:ring-primary/40" />
                <button onClick={addTag} className="px-4 py-2 bg-primary text-white text-sm rounded-lg">Add</button>
              </div>
            </div>
            <div>
              ...
              {form.faq.map((item, i) => (
                <div key={i} className="bg-[var(--color-surface)] rounded-xl p-4 mb-3 space-y-2">
                  ...
                  {form.faq.length > 1 && <button onClick={() => updateForm('faq', form.faq.filter((_, idx) => idx !== i))} className="text-xs text-error">Remove</button>}
                </div>
              ))}
              <button onClick={addFaq} className="text-xs text-primary">+ Add FAQ</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="flex items-center justify-between mt-8 pt-6 border-t border-[var(--color-border)]">
        ...
        <div className="flex gap-3">
          {currentStep < steps.length - 1 ? (
            ...
            <motion.button variants={buttonVariants} initial="idle" whileHover="hover" whileTap="tap" onClick={handleSave} disabled={saving} className="flex items-center gap-1 px-5 py-2 rounded-lg bg-success text-white text-sm font-semibold hover:bg-green-600 transition-colors disabled:opacity-50">
